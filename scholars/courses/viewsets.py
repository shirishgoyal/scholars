...
from .serializers import CourseSerializer, SlideSerializer, CreateCourseSerializer, CategorySerializer, \
    ...
    CreateSlideReviewSerializer, SlideReviewSerializer
...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def reject(self, request, *args, **kwargs):
        ...
        members = instance.members.filter(member=request.user)

        phases = {
            Course.PHASE.reading: 'reading_at',
            Course.PHASE.discussion: 'discussion_at',
            Course.PHASE.slides: 'slides_at',
            Course.PHASE.peer_review: 'peer_review_at',
            Course.PHASE.audio: 'audio_at',
            Course.PHASE.refine: 'refine_at',
            Course.PHASE.pending_approval: 'pending_approval_at',
        }

        if instance.status == Course.STATUS.in_progress and members.count() > 0 and members.first().is_dri:
            # todo add more checks
            if not (instance.phase in [Course.PHASE.onboarding, Course.PHASE.reading]):
                setattr(instance, phases[instance.phase], None)
                setattr(instance, phases[instance.phase - 1], None)
    ...
    def approve(self, request, *args, **kwargs):
        ...
        if instance.status == Course.STATUS.in_progress and members.count() > 0 and members.first().is_dri:
            # todo add more checks
            if instance.phase != Course.PHASE.pending_approval:
                setattr(instance, phases[instance.phase], datetime.now())
                instance.phase = instance.phase + 1
                instance.save()
...
class SlideViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if 'slides' in serializer.validated_data:
            del serializer.validated_data['slides']
    ...
    def approve(self, request, *args, **kwargs):
        ...
        members = instance.course.members.filter(member=request.user)

        if instance.status == Slide.STATUS.pending_approval and members.count() > 0 and members.first().is_dri:
    ...
    def add_feedback(self, request, *args, **kwargs):
        self.serializer_class = SlideSerializer
        slide = self.get_object()

        serializer = CreateSlideReviewSerializer(data=request.data, context={'request': request})
        self.permission_classes = (IsAuthenticated,)

        serializer.is_valid(raise_exception=True)
        instance = serializer.save(slide=slide, user=self.request.user)

        serializer = SlideReviewSerializer(instance=instance, context={'request': request})
        return Response(serializer.data, status.HTTP_200_OK)


class SlideReviewViewSet(viewsets.ModelViewSet):
    ...
    @detail_route(methods=['put'], url_path='resolved')
    def resolved(self, request, *args, **kwargs):
        instance = self.get_object()

        members = instance.slide.course.members.filter(member=request.user)

        if instance.status == SlideReview.STATUS.proposed and members.count() > 0 and members.first().is_dri:
            instance.status = SlideReview.STATUS.resolved
            instance.save()

        serializer = self.get_serializer(instance=instance, context={'request': request})
        return Response(serializer.data, status.HTTP_200_OK)

    @detail_route(methods=['put'], url_path='proposed')
    def proposed(self, request, *args, **kwargs):
        instance = self.get_object()

        members = instance.slide.course.members.filter(member=request.user)

        if instance.status == SlideReview.STATUS.resolved and members.count() > 0 and members.first().is_dri:
            instance.status = SlideReview.STATUS.proposed
            instance.save()
